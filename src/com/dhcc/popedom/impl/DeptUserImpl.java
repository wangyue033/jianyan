 ...
 *
 */
public class DeptUserImpl {
	Page page = new Page();
	
	public List<DeptUser> getDeptUserList(PageBean pageData) throws Exception {
	...
	public String getDeptUserList(DeptUser bean1,String curPage, String perNumber,
	...
	public void addDeptUser(DeptUser bean) throws Exception {
		...
		sqls[0] = "insert into hr_staff (serial_id,job_id,company_id,user_id," +
							...
							"cert_no,tel_phone,work_desc,birth_date,photo_path,sign_path,email,mobile_phone) " +
							"values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		params[0] = new String[] { 
				...
				bean.getBirthDate(),bean.getPhotoPath(),bean.getSignPath(),bean.getEmail(),
				bean.getMobilePhone()
				};
	...
	public Map<String, Object> editDeptUser(DeptUser bean) throws Exception {
		String userId=bean.getUserId();
		String password = DCiperTools.getDigest(userId);
		String[] sqls = new String[2];
		String[][] params = new String[2][];
		sqls[0] = "update hr_staff set serial_id=?,company_id=?," +
				...
				"sign_path=?,password=?,question=?,answer=? where serial_id=?";
		params[0] = new String[] { 
				...
				bean.getWorkDesc(),bean.getBirthDate(),bean.getPhotoPath(),bean.getSignPath(),
	...
	public Map<String, Object> getDeptUserById(String serialId) throws Exception {
		String sql = "select a.serial_id,c.job_name,a.company_id,a.job_id," +
				...
				"a.email,a.tel_phone,a.mobile_phone,a.work_desc,a.birth_date,a.photo_path,a.sign_path," +
		...
		if (crs.next()) {
			...
			bean.setSignPath(crs.getString("sign_path"));
			map.put("record", bean);
		} else {
	...
	public  JSONArray getJobList(String companyId) {
		JSONArray json = null;
		try {
			List<HashMap<String, String>> list = new ArrayList<HashMap<String, String>>();
			String sql = "select job_id ,job_name from hr_job where company_id = ? ";
			CachedRowSetImpl crs = DBFacade.getInstance().getRowSet(
					sql.toString(), new String[] { companyId });
	...
	* 获取检测人员弹框的人员信息
	*/
	public List<ZTreeBean> getDeptUserAllRoot(String companyId,String companyName) {
